...
public extension PageTurnType {
    ...
    nonisolated(unsafe) static let regularDefault = Self.regular(pageTurnDelta: 0.3)
    nonisolated(unsafe) static let earlyCutOffDefault = Self.earlyCutoff(config: .default)

}

public extension EarlyCutOffConfiguration {

    nonisolated(unsafe) static let `default` = EarlyCutOffConfiguration(
